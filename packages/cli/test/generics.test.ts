...
const positive = "tests/types/generic-foundations.ko";
const negative = "tests/types/generic-arity.ko";
...
function temporary(action: (directory: string) => void): void {
  const prefix = join(tmpdir(), "koda-generics-cli-");
...
describe("generic CLI contracts", () => {
  test("check produces only JSON with source-located generic diagnostics", () => {
    const result = run(["check", negative, "--json"]);
    assert.equal(result.status, 1);
    assert.equal(result.stderr, "");
    const output = JSON.parse(result.stdout);
    assert.equal(typeof output.schemaVersion, "string");
    assert.equal(output.diagnostics[0].code, "KODA-T0011");
    assert.equal(output.diagnostics[0].primary.span.startLine, 7);
    assert.equal(output.diagnostics[0].primary.span.file.replaceAll("\\", "/"), negative);
  });

  test("generic foundation builds are identical and the empty entry executes", () => {
    temporary((directory) => {
      ...
      const executed = spawnSync(process.execPath, [join(first, "generic-foundations.entry.mjs")], { encoding: "utf8" });
      assert.equal(executed.status, 0, executed.stderr);
      assert.equal(executed.stdout, "");
    });
  });

  test("failed generic builds create no artifacts and preserve a previous build", () => {
    temporary((directory) => {
